...
public class NotificationService {
	...
	public void sendLoanApplicationNotification(LoanStatusNotificationDTO loanApplicationEvent, ExecutionContext context) {
        // Read the Logic App Workflow URL directly from environmental configurations
        RestClient restClient = RestClient.create();
        ...
        ObjectMapper objectMapper = new ObjectMapper();
        String payload = null;
        var email = buildEmail(loanApplicationEvent.getEmail(), buildLoanAppEmail(loanApplicationEvent), "Your loan application status has changed for applicaation " + loanApplicationEvent.getLoanId());
        try {
			payload = objectMapper.writeValueAsString(email);
		} catch (JsonProcessingException e1) {
			// TODO Auto-generated catch block
	...
	private String buildLoanAppEmail(LoanStatusNotificationDTO loanApplicationEvent) {
		StringJoiner sj = new StringJoiner(" ");
		sj.add("Dear").add(loanApplicationEvent.getCustomerName()).add("\nYour loan application status has been updated to")
		.add(loanApplicationEvent.getStatus())
		.add("and you can view the changes by logging into net banking.");
		return sj.toString();
	}
