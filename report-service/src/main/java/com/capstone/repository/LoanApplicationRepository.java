...
public interface LoanApplicationRepository extends JpaRepository<LoanApplication, Long> {
    ...
    @Query(value = "SELECT SUBSTRING(CONVERT(VARCHAR, CREATED_DATE, 120), 1, 7) AS month_str, " +
                   "COUNT(APPLICATION_ID) AS app_count, " +
                   "SUM(APPLIED_AMOUNT) AS total_vol " +
                   "FROM loan_applications GROUP BY SUBSTRING(CONVERT(VARCHAR, CREATED_DATE, 120), 1, 7)", 
           nativeQuery = true)
